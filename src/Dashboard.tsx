...
export default function ExecutiveDashboard() {
  ...
  const fetchData = () => {
    ...
    Papa.parse(SHEET_CSV_URL, {
      ...
      complete: (results) => {
        ...
        const rows = results.data.slice(1).filter(r => r[COL.PO_NUMBER] && r[COL.PO_NUMBER].trim() !== "");
        setRawRows(rows);
  ...
  const supplierScores = useMemo(() => {
    ...
    filtered.forEach(r => {
      ...
      if (!sup) return;
      if (!map[sup]) map[sup] = {
        name: sup, total: 0, delivered: 0, incomplete: 0,
        undelivered: 0, onTime: 0, cancelled: 0,
        totalAmt: 0, totalDays: 0, delayCount: 0,
      };
      const m = map[sup];
      m.total++;
      ...
      if (days > 0) { m.totalDays += days; m.delayCount++; }
    });
    ...
    return Object.values(map).map(m => {
      ...
      if (score >= 80)      { grade = "A"; gradeColor = COLORS.success; }
      else if (score >= 60) { grade = "B"; gradeColor = COLORS.teal;    }
      else if (score >= 40) { grade = "C"; gradeColor = COLORS.warning; }
      else                  { grade = "D"; gradeColor = COLORS.danger;  }
      return { ...m, deliveryPct: +(deliveryRate*100).toFixed(1),
        onTimePct: +(onTimeRate*100).toFixed(1),
        avgDelay: Math.round(avgDelay), score, grade, gradeColor };
  ...
  const sortedScores = useMemo(() => {
    let rows = supplierScores.filter(s =>
      !scoreSearch || s.name.toLowerCase().includes(scoreSearch.toLowerCase())
    );
    rows = [...rows].sort((a, b) => {
  ...
  const toggleSort = (col) => {
    ...
    else { setScoreSort(col); setScoreSortDir("desc"); }
  };
      ...
      {!loading && !error && (
        <>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 16, marginBottom: 24 }}>
            <KPICard title="Total PO Items"    value={totalItems.toLocaleString()}  sub="Line items in selection"         color={COLORS.primary} icon="📋" />
            <KPICard title="Total Amount"       value={fmt(totalAmount, "₱")}         sub="Sum of all PO amounts"           color={COLORS.info}    icon="💰" />
            <KPICard title="Delivery Rate"      value={`${deliveryRate}%`}             sub={`${deliveredCount} fully delivered`}   color={COLORS.success} icon="✅" />
            <KPICard title="On-Time Rate"       value={`${onTimeRate}%`}               sub={`${onTimeCount} early or on time`}  color={COLORS.teal}    icon="⏱️" />
          </div>
            ...
            {Object.entries(STATUS_COLORS).map(([status, color]) => {
              ...
              return (
                <div key={status} style={{
                  ...
                  <p style={{ margin: 0, fontSize: 12, color: "#6b7280" }}>{fmt(amt, "₱")} total</p>
                </div>
              );
            })}
          </div>
          ...
          <div style={{ marginTop: 28, background: "#fff", borderRadius: 12, boxShadow: "0 1px 6px rgba(0,0,0,0.07)", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", gap: 12, padding: "10px 20px", borderBottom: "1px solid #f1f5f9", flexWrap: "wrap" }}>
              {[["A","80–100",COLORS.success],["B","60–79",COLORS.teal],["C","40–59",COLORS.warning],["D","0–39",COLORS.danger]].map(([g,range,c]) => (
            ...
            <div style={{ padding: "16px 20px 8px", borderBottom: "1px solid #f1f5f9" }}>
              <p style={{ margin: "0 0 10px", fontSize: 12, fontWeight: 600, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.04em" }}>Top 10 Suppliers by Score</p>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={scoreChartData} layout="vertical" margin={{ left: 8, right: 40 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} />
                ...
                <tbody>
                  {sortedScores.map((s, i) => (
                    <tr key={i} style={{ background: i % 2 === 0 ? "#fafafa" : "#fff", borderBottom: "1px solid #f1f5f9" }}>
                      ...
                      <td style={{ padding: "9px 12px", textAlign: "right", color: "#374151" }}>{s.total}</td>
                      ...
                      <td style={{ padding: "9px 12px", textAlign: "right", color: s.avgDelay > 100 ? COLORS.danger : s.avgDelay > 30 ? COLORS.warning : COLORS.success }}>
                      ...
                      <td style={{ padding: "9px 12px", textAlign: "right" }}>
                        <span style={{ ...badgeStyle(s.gradeColor), fontSize: 14, fontWeight: 800, padding: "2px 10px" }}>{s.grade}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                ...
                <tbody>
                  {filtered.slice(0, 200).map((row, i) => {
                    ...
                    return (
                      <tr key={i} style={{ background: i % 2 === 0 ? "#fafafa" : "#fff", borderBottom: "1px solid #f1f5f9" }}>
                        <td style={{ padding: "8px 12px", whiteSpace: "nowrap", color: "#64748b" }}>{parseYear(row[COL.SOURCE_TAB])}</td>
                        ...
                        <td style={{ padding: "8px 12px", whiteSpace: "nowrap", textAlign: "right" }}>₱{Number(row[COL.PRICE]).toLocaleString()}</td>
                        <td style={{ padding: "8px 12px", whiteSpace: "nowrap", textAlign: "right" }}>{row[COL.QTY_ORDER]}</td>
                        <td style={{ padding: "8px 12px", whiteSpace: "nowrap", textAlign: "right" }}>{row[COL.QTY_DELIVERED] || "—"}</td>
